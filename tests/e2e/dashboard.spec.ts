...
test.describe('Dashboard Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/');
    // Wait for dashboard content, the ready-to-run state (definitions but no
    // runs yet), or the first-run experience (empty workspace).
    const pageReady = page.locator('[data-testid="dashboard-page"]')
      .or(page.locator('[data-testid="ready-to-run"]'))
      .or(page.locator('[data-testid="first-run-experience"]'));
    await expect(pageReady.first()).toBeVisible({ timeout: 30000 });
  });
  ...
  test('should display dashboard, ready-to-run, or first-run experience', async ({ page }) => {
    ...
    if (hasDashboard) {
      ...
      // Definitions exist but nothing has run yet (PR #399): a focused
      // "Ready to run" state, not the full leaderboard and not onboarding.
      expect(hasReadyToRun).toBeTruthy();
      await expect(page.locator('[data-testid="ready-to-run"]')).toContainText('Ready to run');
    }
  });
