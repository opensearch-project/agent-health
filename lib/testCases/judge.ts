 ...
 * `judge` into `TestFixtures` via `bindJudge({ evaluatorId: run.evaluatorId, model: bedrockModelId })`.
 * Code that destructures `judge` from the fixture
 * (`async ({ judge }) => ...`) automatically picks up the run's evaluator
 * with no per-call argument. Code that imports `judge` from the package
 * gets the unbound version and must pass `evaluatorId` explicitly.
 */
...
export interface JudgeOptions {
  ...
  model?: string;
  /**
   * Stored evaluator id. Forwarded as `evaluatorId` on the request
   * body. The server resolves built-in (system) evaluators via
   * `getSystemEvaluatorById(...)` — these ids are prefixed `system-`
   * (e.g. `system-rca-default`, `system-factuality`). Any other id is
   * treated as a storage id and resolved via
