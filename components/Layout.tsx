...
const SidebarCollapseContext = createContext<SidebarCollapseContextType | null>(
  null,
);
...
const navItemsAfterEvaluation = [
  {
    ...
    icon: Wand2,
    label: "Skills",
    tooltip: "Evaluate and improve AgentSkills",
    testId: "nav-skills",
  },
  {
    to: "/coding-agents",
    icon: BarChart3,
    label: "AI Dev Tools",
    tooltip: "Claude Code, Kiro & Codex analytics",
    testId: "nav-coding-agents",
  },
  {
    to: "/assistant",
    icon: MessageSquare,
    label: "Assistant",
    tooltip: "AI assistant for help and analysis",
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  // Chrome-vertical-tabs-style hover-open: when the desktop sidebar is pinned
  // collapsed, mouse hover or keyboard focus temporarily expands it as an
  // overlay without reflowing content. Mobile uses the modal Sheet below.
  const [isHoverExpanded, setIsHoverExpanded] = useState(false);
  const hoverTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const hoverZoneRef = useRef<HTMLDivElement>(null);
  const isMouseOverRef = useRef(false);
  // Only keyboard focus that actually opened an already-collapsed rail owns
  // this latch; residual focus after a mouse click must not hold it open.
  ...
  // Mobile navigation is always full-width even if the persisted desktop rail
  // preference is collapsed.
  const collapsed = isCollapsed && !isHoverExpanded && !mobileNavOpen;
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">
                      ...
                      <SidebarMenuSub className="ml-4 mt-1 space-y-1">
                        <SidebarMenuSubItem>
                          <SidebarMenuSubButton
                            asChild
                            isActive={
                              location.pathname === "/evaluations/benchmarks" ||
                              (location.pathname.startsWith(
                                "/evaluations/benchmarks/",
                              ) &&
                                !/\/evaluations\/benchmarks\/[^/]+\/runs\/[^/]+/.test(
                                  location.pathname,
                                ))
                            }
                            data-testid="nav-evals3-benchmarks"
                            className="h-8"
                          >
                            <Link
                        ...
                        <SidebarMenuSubItem>
                          <SidebarMenuSubButton
                            ...
                            data-testid="nav-evals3-test-cases"
                            className="h-8"
                          >
                            <Link
                        ...
                        <SidebarMenuSubItem>
                          <SidebarMenuSubButton
                            asChild
                            isActive={
                              location.pathname === "/evaluations/runs" ||
                              /\/evaluations\/benchmarks\/[^/]+\/runs\/[^/]+/.test(
                                location.pathname,
                              )
                            }
                            data-testid="nav-evals3-runs"
                            className="h-8"
                          >
                            <Link to="/evaluations/runs" className="text-xs">
                              Evaluation Runs
                            </Link>
                          </SidebarMenuSubButton>
                        </SidebarMenuSubItem>
                        <SidebarMenuSubItem>
                          <SidebarMenuSubButton
                            ...
                            data-testid="nav-evaluators"
                            className="h-8"
                          >
                            <Link to="/evaluators" className="text-xs">
                              Evaluators
                            </Link>
                          </SidebarMenuSubButton>
                        </SidebarMenuSubItem>
              ...
              {navItemsAfterEvaluation
                .filter(
                  (item) =>
                    item.to !== "/coding-agents" ||
    ...
    <SidebarCollapseContext.Provider value={{ isCollapsed, setIsCollapsed }}>
      <SidebarProvider className="h-screen overflow-hidden">
        <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
          <SheetContent
            side="left"
            className="w-[180px] border-0 bg-transparent p-0 lg:hidden [&>button]:hidden"
            data-testid="mobile-navigation"
          >
            <SheetTitle className="sr-only">Navigation</SheetTitle>
            <SheetDescription className="sr-only">
              AgentHealth navigation menu
            </SheetDescription>
            <div className="relative h-full w-[180px]">{navigationSidebar}</div>
          </SheetContent>
        </Sheet>
        {/* Desktop hover zone reserves the rail width so flyout expansion does not reflow content. */}
        <div
          ref={hoverZoneRef}
          className="relative hidden h-screen flex-shrink-0 transition-[width] duration-200 lg:block"
          ...
          onKeyDown={(event) => {
            if (event.key !== "Escape" || !isCollapsed || !isHoverExpanded) return;
            isKeyboardOpenRef.current = false;
            if (hoverTimer.current) clearTimeout(hoverTimer.current);
            setIsHoverExpanded(false);
          }}
