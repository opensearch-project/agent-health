...
interface ScopeState {
  ...
  /** Errors recorded while running `beforeAll` — replayed onto every test in the scope. */
  beforeAllResults: MatcherResult[];
...
export class HookOrchestrator implements IHookOrchestrator {
  ...
  /**
   * Index from scopeKey → its describePath (`undefined` for file-level).
   * Cached so we don't repeatedly split describe paths.
   */
  private readonly scopeDescribePaths = new Map<string, string | undefined>();
    ...
    for (const desc of testDescriptors) {
      for (const [key, describePath] of chainFor(desc)) {
        ...
        if (!scope) {
          ...
          this.scopeDescribePaths.set(key, describePath);
        }
        scope.remaining++;
      }
    }
